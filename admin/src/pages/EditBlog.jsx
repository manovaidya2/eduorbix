...
const EditBlog = () => {
  ...
  const addResizeHandles = (image) => {
    ...
    if (!container.classList.contains('image-resize-container')) {
  ...
  const handleResize = (e) => {
    ...
    const { image, container, aspectRatio } = resizeRef.current;
  ...
  // Editor image upload with link option
  const handleEditorImageUpload = (e) => {
    const file = e.target.files[0];
    if (!file) return;
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          {selectedImage && (
            <div className="bg-green-50 border border-green-200 rounded-lg p-3 text-sm text-green-700 flex items-center gap-2">
              ...
              <span>
                <strong>Image selected!</strong> You can:
          ...
          {/* Image Link Tools - Separate Section */}
          <div className="border-2 border-blue-200 rounded-xl p-4 bg-blue-50">
            <h3 className="text-sm font-semibold text-blue-800 mb-3">🔗 Image Link Tools</h3>
            <div className="flex flex-wrap gap-3">
              <button 
                type="button" 
              ...
              >
                <span>🔗</span> Add Link to Selected Image
              </button>
              <button 
                type="button" 
              ...
              >
                <span>🗑️</span> Remove Image Link
              </button>
            </div>
            <p className="text-xs text-blue-600 mt-2">
              Tip: Click on any image in the editor to select it, then use these buttons
            </p>
          </div>
          ...
          <div
            ref={editorRef}
            contentEditable
            className="min-h-[400px] border border-gray-300 rounded-xl p-4 mt-2 focus:outline-none focus:ring-2 focus:ring-green-400 overflow-auto prose max-w-none"
            suppressContentEditableWarning={true}
            style={{ whiteSpace: "pre-wrap" }}
          ></div>
